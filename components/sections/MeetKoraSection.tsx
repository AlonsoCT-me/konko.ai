...
const featureKeys = ['scheduling', 'loyalty', 'efficiency', 'analytics', 'implementation'] as const;
      ...
      <div className={s.meetContainer}>
        ...
        <div className={s.meetGrid}>
          {features.map((feature, i) => (
            <div key={i} className={cn('rounded-3xl overflow-hidden p-6 lg:p-8 flex flex-col', feature.bg)}>
              <div className="flex-1">
                <span className={cn(
                  'inline-block text-xs font-semibold px-3 py-1 rounded-full mb-4',
                  feature.dark ? 'bg-white/10 text-white' : 'bg-brand-gold/15 text-brand-gold-dark'
                )}>
                  {feature.tag}
                </span>
                <h3 className={cn('text-xl font-bold leading-snug mb-3', feature.dark ? 'text-white' : 'text-neutral-950')}>
                  {feature.title}
                </h3>
                <p className={cn('text-sm leading-relaxed', feature.dark ? 'text-neutral-400' : 'text-neutral-500')}>
                  {feature.description}
                </p>
              </div>
